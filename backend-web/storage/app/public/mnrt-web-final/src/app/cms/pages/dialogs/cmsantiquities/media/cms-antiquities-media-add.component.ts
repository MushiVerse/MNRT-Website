...
@Component({
  selector: 'mnrt-cms-antiquities-media-add',
  templateUrl: './cms-antiquities-media-add.component.html',
  styleUrls: ['./cms-antiquities-media-add.component.scss']
})
export class CmsAntiquitiesMediaAddComponent implements OnInit {

  constructor(public matDialogRef: MatDialogRef<CmsAntiquitiesMediaAddComponent>, private spinner: SpinnerService, private cmsService: CmsserviceService, private message: MessagesService, private httpClient: HttpClient) { }
  ...
  onSubmit(){
   ...
   siku=moment(dateTime).format('MM-DD-YYYY HH:mm');
    ...
    let antiqObj = '{"image_slider_description":null,'+
    '"image_slider":null,'+
    '"media_title": null,'+
    ...
    '"card_link": null,'+
    '"card_img": null,'+
    '"about_description":"'+this.AntiquitiesForm.value.des.replace(/"/g, "'")+'",'+
    '"about_img": "/assets/images/antiquities/'+this.fileName+'",'+
    '"about_link": null,'+
...
addAntiq(obj:any){
  ...
  this.cmsService.addAntiquitiesAbout(obj).subscribe(result=>{
...
attachFile(formData:any){
  ...
  this.cmsService.attachNewsPic(formData).subscribe(result=>{
    this.spinner.isCMSLoading.next(false);
    console.log(result);
    this.message.openSnackBar("Details added sucessifully!");
    //this.matDialogRef.close();
