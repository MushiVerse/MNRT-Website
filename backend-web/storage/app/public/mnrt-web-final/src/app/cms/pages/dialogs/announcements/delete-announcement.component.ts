...
@Component({
  selector: 'mnrt-delete-announcement',
...
export class DeleteAnnouncementComponent implements OnInit {
  ...
  row: any = [];
  constructor(@Optional() @Inject(MAT_DIALOG_DATA) public data: any, public matDialogRef: MatDialogRef<DeleteAnnouncementComponent>, private messages: MessagesService, private cmsService: CmsserviceService, public spinner: SpinnerService ) { 
  ...
  onDelete(){
    ...
    this.cmsService.deleteAnnouncementById(this.row.id).subscribe(result=>{
