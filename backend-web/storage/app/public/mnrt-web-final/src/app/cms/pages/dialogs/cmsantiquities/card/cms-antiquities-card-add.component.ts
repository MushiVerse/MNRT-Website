...
@Component({
  selector: 'mnrt-cms-antiquities-card-add',
  templateUrl: './cms-antiquities-card-add.component.html',
  styleUrls: ['./cms-antiquities-card-add.component.scss']
})
export class CmsAntiquitiesCardAddComponent implements OnInit {

  constructor(public matDialogRef: MatDialogRef<CmsAntiquitiesCardAddComponent>, private spinner: SpinnerService, private cmsService: CmsserviceService, private message: MessagesService, private httpClient: HttpClient) { }
...
addAntiq(obj:any){
  ...
  this.cmsService.addAntiquitiesCard(obj).subscribe(result=>{
...
attachFile(formData:any){
  this.spinner.isCMSLoading.next(true);
  this.cmsService.attachAntiquitiesCardPic(formData).subscribe(result=>{
    this.spinner.isCMSLoading.next(false);
    console.log(result);
    this.message.openSnackBar("Card added sucessifully!");
    //this.matDialogRef.close();
